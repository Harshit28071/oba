...
a.max_price as maxPrice, a.max_price as price
FROM `product` a 
left join category b on a.category_id = b.id
left join units c on a.unit_id = c.id
left join units d on a.unit_id = d.id");
$stmt->execute();
$stmt->bind_result($id,$name,$category,$punit,$sunit,$minPrice,$maxPrice,$itemPrice);

while($stmt->fetch()){
    array_push($products,[$id,$name,$category,$punit,$sunit,$itemPrice,$minPrice,$maxPrice]);
}
...
"maxPrice"=>$products[$i][7],"customerPrice"=>$customerPrice,"quantity"=>0,"orderBefore"=>$flag]);
