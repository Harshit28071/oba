...
while($stmt->fetch()){
    array_push($customer,['cid' =>$customer_id, 'cname' =>$customer_name,'cmobile' => $customer_mobile_number,'caddress' =>$customer_address,'cfirm' =>$customer_firm_name,'cGSTIN' =>$customer_GSTIN,'ctype' =>$customer_type,'cdistributor_id' =>$customer_distributor_id,'cstate' => $customer_state,'ccity' =>$customer_city,'distributor_name' => $distributor_name,'state_id' => $state_id,'city_id' => $city_id]);
}
$stmt->close();
$conn->close();
