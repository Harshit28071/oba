...
if(isset($_POST["CustName"])){
    ...
    //Update query
    $SQL_UP ="UPDATE customer SET name = ?,mobile_number = ?,state_id = ?,city = ?,address = ?,GSTIN = ?,type = ?,distributor_id = ? WHERE id = ?";
    $stmt = $conn->prepare($SQL_UP);
    $stmt->bind_param("ssissssii",$customer_name_edit,$customer_mobile_edit,$customer_state_edit,$customer_city_edit,$customer_add_edit,$firm_gstin_edit,$customer_type_edit,$distributor_id_edit,$customer_id);
