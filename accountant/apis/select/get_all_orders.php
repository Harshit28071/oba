...
switch ($status) {
    case "Pending":
        $query = "SELECT orders.id,orders.date,orders.order_status,orders.amount, customer.name,user.username from orders LEFT JOIN customer ON orders.party_id = customer.id LEFT JOIN user ON orders.salesman_id = user.id WHERE (orders.order_status = ?) LIMIT ?,?";
        break;
