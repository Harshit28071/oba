...
//Static Code Start
$tableName = "agarwal_invoices";
if ($id == 2) {
   $tableName = "harihar_invoices";
}
...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM ".$tableName." as invoice where " . $query);
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

// Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(invoice.id) AS allcount FROM ".$tableName." as invoice 
...
$stmt = $conn->prepare("SELECT invoice.id,invoice.date,invoice.status,invoice.amount, customer.name,
   customer.id as customer_id,city.name as city,city.id as city_id,invoice.invoice_number,customer.gstin from ".$tableName." as invoice LEFT JOIN customer ON invoice.party_id = customer.id 
   left join city on customer.city = city.id
